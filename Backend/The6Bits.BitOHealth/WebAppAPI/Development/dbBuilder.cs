    ...
    public class dbBuilder
    {
        ...
        public bool builAccountdDB(string connStr)
        {
            var AccountsStr = "If not exists (select name from sysobjects where name = 'Accounts') " +
                "CREATE TABLE Accounts ( Username VARCHAR(30) primary key NOT NULL, Email VARCHAR(255), " +
        ...
        {
            var AccountsStr = "If not exists (select name from sysobjects where name = 'Logs') CREATE TABLE Logs ( username VARCHAR(30) foreign key references Accounts(username) on delete cascade, description VARCHAR(255), LogLevel VARCHAR(30), LogCategory VARCHAR(30), Date_Time DateTime)";
            var conn = new SqlConnection(connStr);
            using (SqlCommand command = new SqlCommand(AccountsStr, conn))
            {
                conn.Open();
        ...
        {
            var AccountsStr = "If not exists (select name from sysobjects where name = 'VerifyCodes') CREATE TABLE VerifyCodes ( username VARCHAR(30) foreign key references Accounts(username) on delete cascade, " +
        ...
        {
            var AccountsStr = "If not exists (select name from sysobjects where name = 'FailedAttempts') CREATE TABLE FailedAttempts ( Username VARCHAR(30) foreign key references Accounts(username) on delete cascade, Attempts int, Date_Time DateTime)";
            var conn = new SqlConnection(connStr);
            using (SqlCommand command = new SqlCommand(AccountsStr, conn))
            {
                conn.Open();
        ...
        {
            var RecoverysStr = "If not exists (select name from sysobjects where name = 'Recovery') CREATE TABLE Recovery ( Username VARCHAR(30) foreign key references Accounts(username) on delete cascade," +
        ...
        public bool BuildHealthRecorder(string connStr)
        {
            var healthRecorderStr = "If not exists (select name from sysobjects where name = 'HealthRecorder') create table HealthRecorder(record1 varchar(max), record2 varchar(max)," +
                "username varchar(30) foreign key references Accounts(username) on delete cascade,timeSaved DateTime, categoryName varchar(50), recordName varchar(50), primary key (recordName, categoryName))";
            var conn = new SqlConnection(connStr);
        ...
        {
            var RecoveryStr = "If not exists (select name from sysobjects where name = 'WMGoals') CREATE TABLE WMGoals (Username VARCHAR(30) foreign key references Accounts(username) on delete cascade, GoalWeight int, GoalDate DateTime, exerciseLevel int,  CurrentWeight int )";
            var conn = new SqlConnection(connStr);
            using (SqlCommand command = new SqlCommand(RecoveryStr, conn))
            {
                conn.Open();
        ...
        public bool buildFavoriteMedication(string connStr)
        {
            var FavoriteStr = "If not exists (select name from sysobjects where name = 'favoriteMedication') " +
                "CREATE TABLE favoriteMedication (Username VARCHAR(30) foreign key references Accounts(username) on delete cascade, product_ndc VARCHAR(500), " +
        ...
        {
            var RecoveryStr = "If not exists (select name from sysobjects where name = 'Diet')" + "CREATE TABLE Diet (Username VARCHAR(30) foreign key references Accounts(username) on delete cascade, Diet VARCHAR(30), Health VARCHAR(30), Ingr Int, DishType VARCHAR(30),Calories int, CuisineType VARCHAR(30), Excluded VARCHAR(30), MealType VARCHAR(30))";
            var conn = new SqlConnection(connStr);
            using (SqlCommand command = new SqlCommand(RecoveryStr, conn))
            {
                conn.Open();
        ...
        {
            var RecoveryStr = "If not exists (select name from sysobjects where name = 'FavoriteRecipe')" + "CREATE TABLE FavoriteRecipe (Username VARCHAR(30) foreign key references Accounts(username) on delete cascade, Recipe_id VARCHAR(255), DateAdded DateTime)";
            var conn = new SqlConnection(connStr);
            using (SqlCommand command = new SqlCommand(RecoveryStr, conn))
            {
                conn.Open();
        ...
        {
            var RecoveryStr = "If not exists (select name from sysobjects where name = 'Reminders') CREATE TABLE Reminders (R_SK VARCHAR(30), username VARCHAR(30) foreign key references Accounts(username) on delete cascade, name VARCHAR(100), description VARCHAR(1000)," +
