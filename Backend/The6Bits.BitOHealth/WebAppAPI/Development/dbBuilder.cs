...
namespace WebAppMVC.Development
{
    public class dbBuilder
    {
        ...
        {
            var AccountsStr = "If not exists (select name from sysobjects where name = 'VerifyCodes') CREATE TABLE VerifyCodes ( username VARCHAR(30) NOT NULL primary key, time DateTime, code VARCHAR(30), codeType VARCHAR(50))";
            var conn = new SqlConnection(connStr);
            using (SqlCommand command = new SqlCommand(AccountsStr, conn))
            {
                conn.Open();
